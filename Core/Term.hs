...
instance Eq Term where
   ...
   (==) (Fun f) (Fun f') = f==f'
   (==) (Case t bs) (Case t' bs') = (t == t') && (match bs bs') && (all (\ ((Branch _ _ t),(Branch _ _ t')) -> t == t') (zip bs bs'))
...
instance Pretty Term where
   ...
   pretty (Apply e e') = pretty e <+> parens (pretty e')
   pretty (Case e b) = hang (text "case" <+> pretty e <+> text "of") 1 $ vcat $ map pretty b
   pretty e'@(Lambda v e) = let (xs,u) = stripLambda e'
                            in  (text "\\") <> (hsep (map text xs)) <> (text "->") <> (pretty u)
   pretty (Typed e t) = parens (parens (pretty e) <+> text "::" <> text (prettyPrint t))

instance Pretty Branch where
   pretty (Branch c [] t) = (text c) <+> (text "->") <+> (pretty t)
   pretty (Branch c xs t) = let xs' = map (renamevar (free t)) xs
...
renaming (Case t bs) (Case t' bs') s | match bs bs' = (renaming t t' s) >>= (\s -> foldrM (\((Branch _ _ t),(Branch _ _ t')) s -> renaming t t' s) s (zip bs bs'))
...
inst (Case t bs) (Case t' bs') s 
 | match bs bs' = (inst t t' s) >>= (\s->foldrM (\ ((Branch _ _ t),(Branch _ _ t')) s -> inst t t' s) s (zip bs bs'))
...
couple (Case t bs) (Case t' bs') s | match bs bs' = (embedding t t' s) >>= (\s->foldrM (\ ((Branch _ _ t),(Branch _ _ t')) s -> embedding t t' s) s (zip bs bs'))
...
dive t (Case t' bs) s = mplus (embedding t t' s) (msum (map (\(Branch _ vs t') -> embedding t (shift (length vs) 0 t') s) bs))
...
generalise (Case t bs) (Case t' bs') s fv bv | match bs bs' = let (t'',s') = generalise t t' s fv bv
                                                                  (bs'',s'') = foldr (\ ((Branch c xs t),(Branch c' xs' u)) (bs,s) -> let (gv,_) = unzip s
                                                                                                                                          fv' = foldr (\x fv -> let x'=renamevar (gv++fv) x in x':fv) fv xs
                                                                                                                                          xs'' = take (length xs) fv'
                                                                                                                                          t' = foldr (\x t -> subst 0 (Free x) t) t xs''
                                                                                                                                          u' = foldr (\x t -> subst 0 (Free x) t) u xs''
                                                                                                                                          (t'',s') = generalise t' u' s fv' (xs''++bv)
                                                                                                                                      in ((Branch c' xs' $ foldl (\t x -> abstract 0 x t) t'' xs''):bs,s')) ([],s') (zip bs bs')
                                                              in (Case t'' bs'',s'')
...
residualise (Case t bs) fv r d = let (t',d') = residualise t fv r d
                                     (bs',d'') = foldr (\(Branch c xs t) (bs,d) -> let fv' = foldr (\x fv -> let x'=renamevar fv x in x':fv) fv xs
                                                                                       xs' = take (length xs) fv'
                                                                                       (t',d') = residualise (foldr (\x t -> subst 0 (Free x) t) t xs') fv' r d
                                                                                   in  ((Branch c xs $ foldl (\t x -> abstract 0 x t) t' xs'):bs,d')) ([],d') bs
                                 in  (Case t' bs',d'')
...
unfold fs (Case t bs) = Case (unfold fs t) (map (\(Branch c xs t) -> (Branch c xs $ unfold fs t)) bs)
...
rename s (Case t bs) = Case (rename s t) (map (\(Branch c vs t) -> (Branch c vs $ rename s t)) bs)
...
replace' t u (Case t' bs) = Case (replace t u t') (map (\(Branch c xs t') -> (Branch c xs $ replace (shift (length xs) 0 t) (shift (length xs) 0 u) t')) bs)
...
drawLTS' fs args n (Case t bs) = (text ("\"" ++ n ++ "\"[shape=box label=\"\"]\n")) <> (drawLTS' fs args (n ++ "0") t)
                                 ...
                                 <> (hcat (map (\(i,(Branch c xs t)) -> let xs' = map (renamevar (free t)) xs
                                                                            t' = foldr (\x t -> subst 0 (Free x) t) t xs'
