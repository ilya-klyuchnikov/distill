...
transform n (Con c es) EmptyCtx r s fv d = do
                                           ...
                                           return (Con c es')
transform n (Con c es) (AppCtx k e) r s fv d = error ("Constructor application is not saturated: "++show (Con c es))
transform n (Con c es) (CaseCtx k bs) r s fv d = case (find (\(Pattern c' xs,e) -> c==c' && length xs == length es) bs) of
                                                    Nothing -> error ("No matching pattern in case for term:\n\n"++show (Case (Con c es) bs))
                                                    Just (Pattern c' xs,e) -> transform n (foldr (\e e' -> subst 0 e e') e es) k r s fv d
...
transform n (Let x e e') k r s fv d = case (find (\ (x',t) -> e==t) s) of
                                          Just (x',t) -> transform n (subst 0 (Var x') e') k r s fv d
                                          Nothing -> let x' = renamevar fv x
...
transformCtx n e (CaseCtx k bs) r s fv d = do bs' <- mapM (\(Pattern c xs,e') -> let e'' = place e' k
                                                                                 ...
                                                                                 in do t' <- transform n t EmptyCtx r s fv' d
                                                                                       return (Pattern c xs,foldl (\e x -> abstract 0 x e) t' xs')) bs
                                              return (Case e bs')
