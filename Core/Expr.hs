...
renaming (Var x) (Var x') s = if x `elem` fst (unzip s)
...
renaming (Case t bs) (Case t' bs') s | match bs bs' = (renaming t t' s) >>= (\s -> foldrM (\((_,t),(_,t')) s -> renaming t t' s) s (zip bs bs'))
...
inst (Func f) (Func f') s | f == f' = Just s
inst (Case t bs) (Case t' bs') s | match bs bs' = (inst t t' s) >>= (\s->foldrM (\ ((_,t),(_,t')) s -> inst t t' s) s (zip bs bs'))
...
couple (Case t bs) (Case t' bs') s | match bs bs' = (embedding t t' s) >>= (\s->foldrM (\ ((_,t),(_,t')) s -> embedding t t' s) s (zip bs bs'))
...
generalise (Con c ts) (Con c' ts') s fv bv | c==c' && length ts == length ts' = let (ts'',s') = foldr (\(t,t') (ts,s) -> let (t'',s') = generalise t t' s fv bv
                                                                                ...
                                                                                in (Con c ts'',s')
generalise (App t u) (App t' u') s fv bv = let (t'',s') = generalise t t' s fv bv
...
generalise (InfixApp e c f) (InfixApp e' c' f') s fv bv
 | c == c' =
     ...
     in (InfixApp e'' c f'', s'')
generalise (Func f) (Func f') s fv bv | f==f' = (Func f,s)
generalise (Case t bs) (Case t' bs') s fv bv | match bs bs' = let (t'',s') = generalise t t' s fv bv
                                                                  (bs'',s'') = foldr (\ ((Pattern c xs,t),(Pattern c' xs',u)) (bs,s) -> let (gv,_) = unzip s
                                                                                                                                        ...
                                                                                                                                        in ((Pattern c' xs',foldl (\t x -> abstract 0 x t) t'' xs''):bs,s')) ([],s') (zip bs bs')
                                                              in (Case t'' bs'',s'')
...
generalise t t' s fv bv = let xs = intersect (free t) bv
                              ...
                              (gv,_) = unzip s
                              x = renamevar (gv++fv) "x"
                          in (foldr (\t e -> App e t) (Var x) (map Var xs),(x,t''):s)
...
residualise (Case t bs) fv r d = let (t',d') = residualise t fv r d
                                     (bs',d'') = foldr (\(Pattern c xs,t) (bs,d) -> let fv' = foldr (\x fv -> let x'=renamevar fv x in x':fv) fv xs
                                                                                    ...
                                                                                    in  ((Pattern c xs,foldl (\t x -> abstract 0 x t) t' xs'):bs,d')) ([],d') bs
                                 in  (Case t' bs',d'')
...
free' xs (Var x) = if (x `elem` xs) then xs else x:xs
free' xs (Bound _) = xs
...
unfold fs (Case t bs) = Case (unfold fs t) (map (\(p, t) -> (p, unfold fs t)) bs)
...
abstract i b (Var x) = if x==b then Bound i else Var x
...
rename s (Case t bs) = Case (rename s t) (map (\(p, t) -> (p,rename s t)) bs)
